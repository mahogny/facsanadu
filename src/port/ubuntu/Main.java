...
import facsanadu.gui.resource.LabnoteUtil;
...
public class Main
	...
	public static void main(String[] args)
		...
		try
			...
			File dPkg=new File("/tmp/facsanadu");
...
//			File dShareDoc=new File(dShare,"doc");
			...
			File fUsrBinLabstory=new File(dUsrBin,"facsanadu");

			File dCompiledBin=new File("bin");
			File dCompiledLib=new File("lib");
//			File dCompiledLibLinux=new File(dCompiledLib,"linux");
			File dJarTarget=new File(dShare, "facsanadu");
			File dTranslatonTarget=new File(new File(dShare, "facsanadu"),"translations");
...
/*
			for(File f:dCompiledLibLinux.listFiles())
				if(f.getName().endsWith(".jar"))
					copyRecursive(f, new File(dJarTarget,f.getName()));
	*/		
			copyRecursive(new File(dCompiledBin.getParent(),"facsanadu.jar"), new File(dJarTarget,"facsanadu.jar"));
...
//			System.exit(-1);
			...
			Scanner scannerVersion = new Scanner(LabnoteUtil.readFileToString(new File(dCompiledBin,"facsanadu/gui/version.txt")));
			Scanner scannerTimestamp = new Scanner(LabnoteUtil.readFileToString(new File(dCompiledBin,"facsanadu/gui/timestamp.txt")));
			...
			File outDeb=new File(releaseDir, "facsanadu-"+version+".deb");
